 ...
 * Description: A smple plugin to add a SeedBot GPT4 chatbot to your WordPress Website.
 * Version:     1.0
...
function seedbot_gpt4_enqueue_scripts()
{
    ...
    echo "<script>console.log('tesooooot');</script>";

}
add_action('wp_enqueue_scripts', 'seedbot_gpt4_enqueue_scripts');
seedebug_to_console($seedbot_settings);

$testingdeb = 'seedbot_bot_name';
seedebug_to_console($testingdeb);


//Logging in Dev
function seedebug_to_console($data) {
    $output = $data;
    if (is_array($output))
